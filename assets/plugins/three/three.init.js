...
var SEPARATION = 40, AMOUNTX = 100, AMOUNTY = 35;
...
var particles, particle, count = 0 , renderHeight , renderWidth;

var HeaderElement = document.querySelector(".citc_header");
renderHeight = HeaderElement.getBoundingClientRect().height;
renderWidth = HeaderElement.getBoundingClientRect().width;
...
function init() {
	...
	container = document.querySelector('.header-particles'); //document.createElement( 'div' );
	//document.body.appendChild( container );
	if(container) {
    	container.className += container.className ? ' waves' : 'waves';
	}

	camera = new THREE.PerspectiveCamera( 75, renderWidth / renderHeight, 1, 10000 );
	...
	var material = new THREE.SpriteCanvasMaterial( {
		...
		color: 0xaf99f0, //changes color of particles
